#!/usr/bin/env node
/**
 * Fetch Dish Images for Berlin Restaurants
 * Scrapes images from Uber Eats, Wolt, Just Eat and updates dishes in Firestore
 *
 * Usage:
 *   node fetch-berlin-dish-images.cjs                    # Dry run - show what would be fetched
 *   node fetch-berlin-dish-images.cjs --execute         # Actually fetch and update images
...
async function fetchBerlinDishImages() {
  console.log('\n=== FETCH BERLIN DISH IMAGES ===\n');
  ...
  if (SPECIFIC_VENUE) {
    berlinVenues = berlinVenues.filter(v => v.id === SPECIFIC_VENUE);
  }
